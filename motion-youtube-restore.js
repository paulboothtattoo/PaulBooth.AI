(() => {
  ...
  const installStyles = () => {
    if (document.getElementById("pb-motion-youtube-styles")) return;
    const style = document.createElement("style");
    style.id = "pb-motion-youtube-styles";
    style.textContent = `
      ...
      #motion-project-grid .pb-youtube-poster::after {
        content: "▶"; position: absolute; right: 14px; bottom: 14px; z-index: 4;
      ...
      #motion-project-grid .motion-folder-index { z-index: 20 !important; }
  ...
  const render = () => {
    ...
    grid.dataset.pbMotionOwner = "youtube-posters-v3";
  ...
  const start = () => {
    ...
    const ensureRendered = () => {
      ...
      const grid = document.getElementById("motion-project-grid");
      if (!grid) return;
      const owned = grid.dataset.pbMotionOwner === "youtube-posters-v3";
