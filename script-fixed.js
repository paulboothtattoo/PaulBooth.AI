...
(() => {
  ...
  const videoMarkup = (item) => {
    ...
    return `<video controls preload="metadata" playsinline${poster}>
  ...
  const renderRepository = (items) => {
    ...
    const render = () => {
      ...
      grid.innerHTML = visibleItems.map((item) => {
        const directLink = escapeHtml(item.link || item.src);
        const linkText = item.link ? "Visit Project" : "Open File";

        return `<article class="folder-media-card reveal visible">
          <div class="folder-media-preview">
          ...
          <div class="folder-media-copy">
            ...
            ${item.description ? `<p>${escapeHtml(item.description)}</p>` : ""}
            <a href="${directLink}" target="${item.link ? "_self" : "_blank"}" rel="noopener">${linkText} →</a>
          </div>
        </article>`;
      }).join("");
...
if (machineGrid && Array.isArray(data?.machines)) {
  machineGrid.innerHTML = data.machines.map((item, i) => `
  <article class="machine-card reveal">
    <span class="machine-index">0${i + 1}</span>
    <div class="machine-core" aria-hidden="true"></div>
    <h3>${item.title}</h3>
    <p>${item.description}</p>
    <a href="#contact">Prepare content <span>→</span></a>
  </article>`).join('');
}
